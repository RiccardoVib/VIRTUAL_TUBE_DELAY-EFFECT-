...
public:
    VIRTUAL_TUBE_DELAYObj() // constructor
    {
        ...
        //output buffers
        len = 5114; //[46.7755102040816] m
        outBufferL = new float[len];
        outBufferR = new float[len];
        
        outBufferRefL = new float[len];
        outBufferRefR = new float[len];
    ...
    void process(const float* input, float* lengthL, float* lengthR, float* gainL, float* gainR, float* size, float* lengthRefL, float* lengthRefR, float* gainRefL, float* gainRefR, float* dry_wet, float* outputR, float* outputL, int n)
    {
        ...
        float lengL = lengtL/100;
        float lengR = lengtR/100;
        ...
        float lengRefL = lengtRefL/100;
        float lengRefR = lengtRefR/100;
        ...
        float length_totL = 2*lengRefL;
        float length_totR = 2*lengRefR;
        ...
        rptrLeft = (wptrLeft - delaySamplesL);
        if (rptrLeft < 0){rptrLeft += len;}
        rptrRight = (wptrRight - delaySamplesR);
        if (rptrRight < 0){rptrRight += len;}
        
        rptrLeft_Ref = (wptrLeft_Ref - delaySamplesL_tot);
        if (rptrLeft_Ref < 0){rptrLeft_Ref += len;}
        rptrRight_Ref = (wptrRight_Ref - delaySamplesR_tot);
        if (rptrRight_Ref < 0){rptrRight_Ref += len;}
        ...
        for (int i = 0; i < n; ++i)
        {
            ...
            inR[i_0] = (t_sample)input[i];
            inL[i_0] = (t_sample)input[i];
                
            outR[i_0] = b0f_R*inR[i_0] + b1f_R*inR[i_1] + b2f_R*inR[i_2] + b3f_R*inR[i_3] + b4f_R*inR[i_4] + b5f_R*inR[i_5] + b6f_R*inR[i_6] - a1f_R*outR[i_1] - a2f_R*outR[i_2] - a3f_R*outR[i_3] - a4f_R*outR[i_4] - a5f_R*outR[i_5] - a6f_R*outR[i_6];
                
            outL[i_0] = b0f_L*inL[i_0] + b1f_L*inL[i_1] + b2f_L*inL[i_2] + b3f_L*inL[i_3] + b4f_L*inL[i_4] + b5f_L*inL[i_5] + b6f_L*inL[i_6] - a1f_L*outL[i_1] - a2f_L*outL[i_2] - a3f_L*outL[i_3] - a4f_L*outL[i_4] - a5f_L*outL[i_5] - a6f_L*outL[i_6];
                
            
            out2R[i_0] = b0fRef_R*outR[i_0] + b1fRef_R*outR[i_1] + b2fRef_R*outR[i_2] + b3fRef_R*outR[i_3] + b4fRef_R*outR[i_4] + b5fRef_R*outR[i_5] + b6fRef_R*outR[i_6] - a1fRef_R*out2R[i_1] - a2fRef_R*out2R[i_2] - a3fRef_R*out2R[i_3] - a4fRef_R*out2R[i_4] - a5fRef_R*out2R[i_5] - a6fRef_R*out2R[i_6];
                
            out2L[i_0] = b0fRef_L*outL[i_0] + b1fRef_L*outL[i_1] + b2fRef_L*outL[i_2] + b3fRef_L*outL[i_3] + b4fRef_L*outL[i_4] + b5fRef_L*outL[i_5] + b6fRef_L*outL[i_6] - a1fRef_L*out2L[i_1] - a2fRef_L*out2L[i_2] - a3fRef_L*out2L[i_3] - a4fRef_L*out2L[i_4] - a5fRef_L*out2L[i_5] - a6fRef_L*out2L[i_6];
                
                
            out3R[j_0] = br[0]*out2R[i_0] + br[1]*out2R[i_1] + br[2]*out2R[i_2] + br[3]*out2R[i_3] - ar[0]*out3R[j_1] - ar[1]*out3R[j_2] - ar[2]*out3R[j_3];
                
            out3L[j_0] = br[0]*out2L[i_0] + br[1]*out2L[i_1] + br[2]*out2L[i_2] + br[3]*out2L[i_3] - ar[0]*out3L[j_1] - ar[1]*out3L[j_2] - ar[2]*out3L[j_3];
                
                
            outBufferR[wptrRight] = outR[i_0];
            outBufferL[wptrLeft] = outL[i_0];
            
            outBufferRefR[wptrRight_Ref] = out3R[j_0];
            outBufferRefL[wptrLeft_Ref] = out3L[j_0];
            
            outputR[i] = dry*(t_sample)input[i] + wet*(gR*outBufferR[rptrRight] + gRef_R*outBufferRefR[rptrRight_Ref]);
            outputL[i] = dry*(t_sample)input[i] + wet*(gL*outBufferL[rptrLeft] + gRef_L*outBufferRefL[rptrLeft_Ref]);

            
            //updating pointers
            wptrRight = (wptrRight+1);
            if (wptrRight >= len){wptrRight -= len;}
            wptrLeft = (wptrLeft+1);
            if (wptrLeft >= len){wptrLeft -= len;}
                
            wptrRight_Ref = (wptrRight_Ref+1);
            if (wptrRight_Ref >= len){wptrRight_Ref -= len;}
            wptrLeft_Ref = (wptrLeft_Ref+1);
            if (wptrLeft_Ref >= len){wptrLeft_Ref -= len;}
            
            rptrRight = (rptrRight+1);
            if (rptrRight >= len){rptrRight -= len;}
            rptrLeft = (rptrLeft+1);
            if (rptrLeft >= len){rptrLeft -= len;}

            rptrRight_Ref = (rptrRight_Ref+1);
            if (rptrRight_Ref >= len){rptrRight_Ref -= len;}
            rptrLeft_Ref = (rptrLeft_Ref+1);
            if (rptrLeft_Ref >= len){rptrLeft_Ref -= len;}
...
private:
    ...
    int ptr;
    int ptr2;
    int i_0, i_1, i_2, i_3, i_4, i_5, i_6;
    int j_0, j_1, j_2, j_3;
    int rptrLeft, wptrLeft, rptrRight, wptrRight;
    int rptrLeft_Ref, wptrLeft_Ref, rptrRight_Ref, wptrRight_Ref;
...
t_int *VIRTUAL_TUBE_DELAY_tilde_perform(t_int *w)
{
    ...
    t_sample  *lengthRefL = (t_sample *)(w[8]); //lenght ref left
    t_sample  *lengthRefR = (t_sample *)(w[9]); //lenght ref right
    t_sample  *gainRefL = (t_sample *)(w[10]);   //gain ref left
    t_sample  *gainRefR = (t_sample *)(w[11]);   //gain ref right
    t_sample  *dry_wet = (t_sample *)(w[12]);  //dry/wet
    
    
    // 2 signal output: filtered stereo signal
    t_sample *outputL = (t_sample *)(w[13]); //output samples left
    t_sample *outputR = (t_sample *)(w[14]); //output samples right
    
    int n = (int)(w[15]);
    
    x->dd->process(input, lengthL, lengthR, gainL, gainR, size, lengthRefL, lengthRefR, gainRefL, gainRefR, dry_wet, outputR, outputL, n);
